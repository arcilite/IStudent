...
void GTMAssertSelectorNilOrImplementedWithArgs(id obj, SEL sel, ...) {
  ...
  if (obj && sel) {
    ...
    if (![obj respondsToSelector:sel]) {
      //NSLog(@"\"%@\" selector \"%@\" is unimplemented or misnamed",                             NSStringFromClass([obj class]),                             NSStringFromSelector(sel));
      Debugger();
    } else {
      ...
      while ((expectedArgType = va_arg(argList, const char*)) != 0) {
        ...
        if ([sig numberOfArguments] > argCount) {
          ...
          if(0 != strncmp(foundArgType, expectedArgType, strlen(expectedArgType))) {
            //NSLog(@"\"%@\" selector \"%@\" argument %d should be type %s",                  NSStringFromClass([obj class]),                  NSStringFromSelector(sel), (argCount - 2), expectedArgType);
            Debugger();
          }
        }
        argCount++;
      ...
      if (argCount != [sig numberOfArguments]) {
        //NSLog( @"\"%@\" selector \"%@\" should have %d arguments",                       NSStringFromClass([obj class]),                       NSStringFromSelector(sel), (argCount - 2));
        Debugger();
      }
    }
  }
