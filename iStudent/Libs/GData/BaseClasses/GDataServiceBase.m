  ...
  if (object != nil || dataLength == 0) {
    ...
    if ([ticket shouldFollowNextLinks]
      ...
      if (accumulatedFeed) {
        ...
        if (feedPageCount > 2) {
        //NSLog(@"Fetching %@ required following %u \"next\" links; use a query with a larger setMaxResults: for faster feed accumulation",                NSStringFromClass([accumulatedFeed class]),(unsigned int) [ticket nextLinksFollowedCounter]);
        }
#endif
...
- (void)objectFetcher:(GTMHTTPFetcher *)fetcher failedWithData:(NSData *)data error:(NSError *)error {
  ...
  if (dataString) {
   //NSLog(@"serviceBase:%@ objectFetcher:%@ failedWithStatus:%ld data:%@",         self, fetcher, (long) [error code], dataString);
  }
#endif
